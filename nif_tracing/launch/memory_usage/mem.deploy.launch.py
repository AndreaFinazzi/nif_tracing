...
# NIF CONTROL STACK ###############################################
    ...
    nif_csl_node = Node(
        ...
        remappings=[
            ...
            ('in_perception_steering', '/no/in_perception_steering'),
            ('in_wall_distance_inner', '/no/in_wall_distance_inner'),
            ('in_wall_distance_outer', '/no/in_wall_distance_outer'),
    ...
    if track == LOR:
        ...
        gear_track = 'LVMS'
# @TODO Attention: this kind of parameterization should be standardized and wrapped in a launch utility package (py)
...
### NIF TRACING ############################################
    ...
    trace = Trace(
        session_name='memory-usage.deploy.' + date.today().isoformat(),
        events_ust=[
